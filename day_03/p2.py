...
found_inds_filtered = [ found_inds[0]] + [ 
    found_inds[i+1] 
    for i in range(0, len(found_inds) - 1) 
    if found_inds[i][0] != found_inds[i+1][0] 
]

# found_inds_slices = [f'[:{found_inds_filtered[1][1]}]'] + [
#     f"[{found_inds_filtered[i][1]}:{found_inds_filtered[i+1][1]}]"
#     for i in range(2, len(found_inds_filtered) - 1, 2)
# ]
# print(found_inds_slices)
